package com.lockboxth.lockboxkiosk.cardreader;
...
import co.advancedlogic.thainationalidcard.SmartCardMessage;

public class SmartCardDevice {
    private static final String ACTION_USB_PERMISSION = "com.lockboxth.lockboxkiosk.SmartCard.USB_PERMISSION";
    ...
    public static SmartCardDevice getSmartCardDevice(Context context, String nameContain, int infIndex, int endpointInputIndex, int endpointOutputIndex, SmartCardDeviceEvent eventCallback) {
        ...
        for (String key : deviceList.keySet()) {
            ...
            if (deviceList.get(key).getProductName() != null && deviceList.get(key).getProductName().contains(nameContain)) {
...
//            deviceList.get(key).getProductName() != null &&
            if (deviceList.get(key).getProductName() != null && deviceList.get(key).getProductName().contains(nameContain)) {
                device = deviceList.get(key);
                Log.d(TAG, "Found device: " + device.getProductName());
                break;
...
//                    UsbDevice device =  SmartCardDevice.this.device;

                    Log.d(TAG, "device is null :" + (device == null) + "");
...
//                                throw new RuntimeException("Invalid USB device interface or endpoint");
                            ...
                            if (SmartCardDevice.this.eventCallback != null) SmartCardDevice.this.eventCallback.OnReady(SmartCardDevice.this);
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action != null && action.equals(UsbManager.ACTION_USB_DEVICE_DETACHED)) {
                ...
                if (device.getDeviceName().equals(SmartCardDevice.this.device.getDeviceName())) {
                    if (SmartCardDevice.this.eventCallback != null) SmartCardDevice.this.eventCallback.OnDetached(SmartCardDevice.this);
